...
const CasosRealesSection = () => {
  const { t } = useLanguage();
  const cases = [
    { title: t("cases.1.title"), location: t("cases.1.location"), desc: t("cases.1.desc") },
    { title: t("cases.2.title"), location: t("cases.2.location"), desc: t("cases.2.desc") },
    { title: t("cases.3.title"), location: t("cases.3.location"), desc: t("cases.3.desc") },
  ];
      ...
      <div className="container">
        <motion.div variants={scaleReveal} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} className="text-center mb-4">
          ...
          <motion.h2 className="font-heading text-2xl font-bold uppercase tracking-wider md:text-3xl" variants={textReveal} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0}>
            {t("cases.title1")}<span className="text-gradient-gold">{t("cases.title2")}</span>
          </motion.h2>
        ...
        <motion.div className="mx-auto grid max-w-5xl gap-6 md:grid-cols-3" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          {cases.map((c) => (
