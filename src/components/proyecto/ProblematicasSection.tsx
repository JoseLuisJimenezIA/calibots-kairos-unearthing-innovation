...
const problems = [
  { area: "Cultural", desc: "Aquí surge un desafío ético fundamental: desde la intervención de sitios arqueológicos que las comunidades locales consideran sagrados, donde el manejo de restos humanos y objetos ancestrales no es solo un proceso técnico, sino una acción que impacta directamente en la identidad y las creencias de poblaciones que ven en esos hallazgos a sus propios antepasados.", color: "text-primary" },
  { area: "Económica", desc: "Este es otro de los grandes desafíos, ya que desde el ámbito de la minería sí se efectúa buena inversión debido a la retribución, sin embargo la arqueología y similares no cuentan con mucha inversión debido a la retribución económica que puede generar.", color: "text-primary" },
  { area: "Seguridad", desc: "Dentro de las excavaciones podemos encontrar materiales poco rígidos con precipitaciones, lugares de difícil acceso y también con condiciones de aire contaminado por diferentes tipos de gases, ocasionando problemas respiratorios a corto y largo plazo.", color: "text-accent" },
  { area: "Tecnológica", desc: "Aunque existen herramientas como el LIDAR o el Georradar (GPR), estas tecnologías a menudo enfrentan dificultades en terrenos con alta densidad de vegetación, humedad extrema o suelos con alta mineralización. También están los sistemas robóticos con retroalimentación háptica y un tercer enfoque es la interoperabilidad: cómo procesar y almacenar modelos 3D de forma accesible para investigadores de todo el mundo.", color: "text-secondary" },
];
  ...
  <section className="section-dark py-16">
    <div className="container">
      ...
      <div className="mx-auto grid max-w-4xl gap-6 md:grid-cols-2">
        {problems.map((p, i) => (
          <motion.div key={p.area} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i + 1} className="glass-card-hover p-6">
      ...
      <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={5} className="mx-auto mt-10 max-w-3xl glass-card p-6">
        ...
        <ul className="list-inside list-disc space-y-1.5 font-subtitle text-sm text-muted-foreground">
