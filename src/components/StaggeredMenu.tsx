...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const handleItemClick = useCallback((e: React.MouseEvent, link: string) => {
    e.preventDefault();
    closeMenu();
    setTimeout(() => navigate(link), 350);
  }, [closeMenu, navigate]);
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#1A1308', '#D4A017'];
          let arr = [...raw];
          if (arr.length >= 3) {
