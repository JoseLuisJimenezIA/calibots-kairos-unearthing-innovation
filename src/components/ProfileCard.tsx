...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const cardStyle = useMemo(() => ({
    ...
    '--pc-inner-gradient': innerGradient ?? 'linear-gradient(145deg, hsla(25, 30%, 10%, 0.6) 0%, hsla(40, 76%, 50%, 0.15) 100%)',
    '--pc-behind-glow-color': behindGlowColor ?? 'hsla(40, 76%, 50%, 0.5)',
        ...
        <div className="pc-card">
          ...
          <div className="pc-content">
            <div className="pc-details">
              <h3>{name}</h3>
              <p className={isCoach ? 'coach-role' : ''}>{title}</p>
            </div>
          </div>
          {/* Role badge at bottom */}
          <div className={`pc-role-badge ${isCoach ? 'coach' : ''}`}>
            <div className="pc-role-dot" />
            <span>{isCoach ? 'Coach' : 'Integrante'}</span>
          </div>
        </div>
      </div>
    </div>
