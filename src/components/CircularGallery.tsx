...
class Media {
  ...
  speed: any; isBefore: any; isAfter: any;
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          float edgeSmooth = 0.002; float alpha = 1.0 - smoothstep(-edgeSmooth, edgeSmooth, d);
  ...
  update(scroll: any, _direction: string) {
    ...
    if (_direction === 'right' && this.isBefore) { this.extra -= this.widthTotal; this.isBefore = this.isAfter = false; }
    if (_direction === 'left' && this.isAfter) { this.extra += this.widthTotal; this.isBefore = this.isAfter = false; }
  }
...
class GalleryApp {
  ...
  createMedias(items: any[], bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = Array.from({ length: 12 }, (_, i) => ({
      image: `https://picsum.photos/seed/${i + 1}/800/600?grayscale`, text: `Image ${i + 1}`
    }));
