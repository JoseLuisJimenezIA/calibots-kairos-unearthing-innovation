...
import { textReveal, slideFromLeft, slideFromRight } from "@/lib/animations";
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <section className="relative py-16 sm:py-24 overflow-hidden section-dark">
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <motion.div className="h-[400px] w-[400px] rounded-full bg-accent/8 blur-[100px]" animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }} />
        </div>
        <div className="container relative z-10 text-center">
          <motion.h1 className="font-heading text-3xl font-black uppercase tracking-wider md:text-5xl lg:text-6xl" variants={textReveal} initial="hidden" animate="visible" custom={0}>
            <span className="text-gradient-crimson">{t("ct.title")}</span>
          </motion.h1>
        ...
        <div className="container">
          <div className="grid gap-12 md:grid-cols-2">
            <motion.div variants={slideFromLeft} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0}>
              <h2 className="mb-8 font-heading text-lg font-bold uppercase tracking-wider">{t("ct.form.title")}</h2>
              <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
                <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}>
                  <Label htmlFor="name" className="font-subtitle text-sm uppercase tracking-wider">{t("ct.form.name")}</Label>
                  <Input id="name" placeholder={t("ct.form.name.ph")} className="mt-2 bg-card/50 border-primary/20 font-subtitle" />
                </motion.div>
                <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
                  <Label htmlFor="email" className="font-subtitle text-sm uppercase tracking-wider">{t("ct.form.email")}</Label>
                  <Input id="email" type="email" placeholder={t("ct.form.email.ph")} className="mt-2 bg-card/50 border-primary/20 font-subtitle" />
                </motion.div>
                <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }}>
                  <Label htmlFor="message" className="font-subtitle text-sm uppercase tracking-wider">{t("ct.form.message")}</Label>
                  <Textarea id="message" placeholder={t("ct.form.message.ph")} rows={5} className="mt-2 bg-card/50 border-primary/20 font-subtitle" />
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }}>
                  <Button type="submit" className="bg-primary text-primary-foreground font-subtitle font-bold uppercase tracking-wider glow-gold hover:scale-105 transition-all duration-300">{t("ct.form.submit")}</Button>
                </motion.div>
              </form>
            </motion.div>
            ...
            <motion.div variants={slideFromRight} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} className="space-y-8">
              <div>
                <h2 className="mb-5 font-heading text-lg font-bold uppercase tracking-wider">{t("ct.info.title")}</h2>
                <ul className="space-y-4 font-subtitle text-muted-foreground">
                  <motion.li className="flex items-start gap-3" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
                    <School className="mt-0.5 h-5 w-5 shrink-0 text-secondary" /><span>{t("ct.info.school")}<br />{t("ct.info.club")}</span>
                  </motion.li>
                  <motion.li className="flex items-start gap-3" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }}>
                    <MapPin className="mt-0.5 h-5 w-5 shrink-0 text-secondary" /><span>{t("ct.info.city")}</span>
                  </motion.li>
              ...
              <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.5 }}>
                <h3 className="mb-4 font-heading text-sm font-bold uppercase tracking-wider">{t("ct.social.title")}</h3>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, filter: "blur(6px)" }} whileInView={{ opacity: 1, scale: 1, filter: "blur(0px)" }} viewport={{ once: true }} transition={{ delay: 0.6, duration: 0.7 }}>
                <h3 className="mb-3 font-heading text-sm font-bold uppercase tracking-wider">{t("ct.location.title")}</h3>
                <div className="flex aspect-video items-center justify-center rounded-lg border border-primary/10 bg-card/50 text-muted-foreground">
                  <div className="text-center"><MapPin className="mx-auto mb-2 h-8 w-8 opacity-40" /><span className="font-subtitle text-sm opacity-60">{t("ct.location.map")}</span></div>
                </div>
              </motion.div>
            </motion.div>
          </div>
