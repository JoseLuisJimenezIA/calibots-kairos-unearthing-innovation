...
const Galeria = () => {
  const { t } = useLanguage();
  const filters = [
    { key: "Todo", label: t("gal.all") },
    { key: "Proyecto", label: t("gal.proyecto") },
    { key: "Equipo", label: t("gal.equipo") },
    { key: "Robot", label: t("gal.robot") },
    { key: "Competencia", label: t("gal.competencia") },
    { key: "Investigación", label: t("gal.investigacion") },
  ];
      ...
      <section className="section-darker py-20">
        <div className="container">
          <motion.div className="mb-12 flex flex-wrap justify-center gap-2" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            {filters.map((f) => (
              <button key={f.key} onClick={() => setActive(f.key)} className={`rounded-full px-5 py-2 font-subtitle text-sm font-semibold uppercase tracking-wider transition-all duration-300 ${active === f.key ? "bg-primary text-primary-foreground glow-gold scale-105" : "border border-primary/20 bg-primary/5 text-muted-foreground hover:bg-primary/10 hover:text-primary hover:scale-105"}`}>{f.label}</button>
            ))}
          </motion.div>
          <motion.div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" layout>
            <AnimatePresence mode="popLayout">
              {filtered.map((item) => (
                <motion.div key={item.id} layout initial={{ opacity: 0, scale: 0.85, filter: "blur(8px)" }} animate={{ opacity: 1, scale: 1, filter: "blur(0px)" }} exit={{ opacity: 0, scale: 0.85, filter: "blur(8px)" }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="cursor-pointer glass-card-hover group" onClick={() => item.type === "photo" && setLightbox(item.id)}>
        ...
        {lightbox !== null && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-background/90 backdrop-blur-xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setLightbox(null)}>
            <motion.div className="relative mx-4 w-full max-w-2xl" initial={{ scale: 0.7, opacity: 0, filter: "blur(10px)" }} animate={{ scale: 1, opacity: 1, filter: "blur(0px)" }} exit={{ scale: 0.7, opacity: 0, filter: "blur(10px)" }} transition={{ ease: [0.16, 1, 0.3, 1] }} onClick={(e) => e.stopPropagation()}>
              ...
              <PhotoPlaceholder aspectRatio="video" label={galleryItems.find((g) => g.id === lightbox)?.label || "📷"} className="text-lg" />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
